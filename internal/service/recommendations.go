...
func (s RecommendationService) Create(ctx context.Context, u domain.User, student, note, request string, items []domain.RecommendationItem) (domain.Recommendation, error) {
	...
	rec, err := s.Repo.Create(ctx, repository.RecommendationInput{ID: id, StudentID: student, CreatedBy: u.ID, Note: note, Items: items}, request)
	if err != nil {
		return domain.Recommendation{}, err
	}
	for _, item := range items {
		if err := s.Plans.ReservePlan(ctx, item.AdmissionPlanID); err != nil {
			return domain.Recommendation{}, err
		}
	}
	return rec, nil
